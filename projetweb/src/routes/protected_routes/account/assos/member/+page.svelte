...
<div class="myBody">
	<div class="myBody-text">
        <p>Choisis ton asso : 
        <select bind:value={selected_asso}>
            {#each list_asso as asso}
                <option value={asso.id} on:change="{() => console.log(selected_asso,asso.name_association)}">
                    {asso.name_association}
                </option>
            {/each}
        </select>
        </p>
        <form id="becomeMemberForm" on:submit|preventDefault={handleSubmit}>
        <div>
            <input
                type="submit"
